...
**갤럭시 Z 폴드처럼 화면이 두 개인 기기를 위해 접었을 때/펼쳤을 때 × 세로/가로 네 가지 설정을 완전히 분리**했습니다.
...
이 모든 값이 **접었을 때 / 펼쳤을 때 × 세로 / 가로 = 4개 프로필**로 각각 따로 저장됩니다.

노치는 기기에 물리적으로 고정돼 있어서 화면을 돌리면 화면 좌표계에서의 위치가 완전히 달라집니다.
(세로에서는 위쪽에 가로로 눕고, 가로에서는 옆쪽에 세로로 섭니다.)
그래서 방향별로 크기·위치를 따로 잡아야 합니다.

- 폴드 상태는 `smallestScreenWidthDp` 로 판별합니다 (기준값 기본 480dp, 앱에서 조절 가능).
- 방향은 `Configuration.orientation` 으로 판별합니다.
- 접거나 펴거나 돌리면 해당 프로필로 자동 전환됩니다.
- 제스처 동작만 4개 화면에 한 번에 복사하는 버튼도 있습니다.

> 가로 모드는 왼쪽으로 돌릴 때와 오른쪽으로 돌릴 때 노치가 반대편 끝에 옵니다.
> 현재는 가로를 하나의 프로필로 취급하므로, 주로 쓰는 회전 방향을 기준으로 맞춰주세요.
...
2. 그때 서비스가 붙어 있지 않으면 **"액션노치가 꺼져 있어요"** 알림을 띄우고,
...
│  └─ ScreenDetector.kt         접힘/펼침 · 세로/가로 판별
├─ service/
